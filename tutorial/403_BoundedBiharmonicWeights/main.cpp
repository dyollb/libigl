...
#ifdef SURFACE_WEIGHTS
	MatrixXd NV;
	MatrixXi NF, I;
	igl::remove_unreferenced(V, F, NV, NF, I);
	igl::writeOFF("F:/Dropbox/Work/Data/Fats_obese_dancing/fats.off", NV, NF);

	std::cerr << "V: " << V.rows() << " x " << V.cols() << std::endl;
	std::cerr << "W: " << W.rows() << " x " << W.cols() << std::endl;
	std::cerr << "NV: " << NV.rows() << " x " << W.cols() << std::endl;
	MatrixXd NW(NV.rows(), W.cols());
	for (Index i=0; i< NV.rows(); ++i)
	{
